    ...
    <div className="sticky top-0 z-40 bg-white/10 backdrop-blur-xl border-b border-white/20">
      <Container className="py-0">
        <nav className="navbar flex-wrap gap-2 sm:gap-0">
          ...
          <div className="navbar-end flex items-center gap-1.5 sm:gap-2 flex-wrap">
            <div ref={dropdownRef} className="relative dropdown dropdown-end">
              <button
                type="button"
                onClick={() => setOpen((v) => !v)}
                className="btn btn-ghost h-9 min-h-0 px-2.5 sm:px-3 py-1.5 sm:py-2 bg-white/15 text-white/80 hover:text-primary hover:bg-primary/25 border border-transparent hover:border-primary/30 rounded-xl backdrop-blur-md transition-all flex items-center gap-2"
                aria-haspopup="menu"
                aria-expanded={open}
              >
                <span className="uppercase text-xs sm:text-sm">{current}</span>
                <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <polyline points="6 9 12 15 18 9"></polyline>
                </svg>
              </button>
              {open && (
                <ul role="menu" className="menu dropdown-content mt-2 p-2 shadow bg-base-200 text-base-content rounded-box w-28 z-[100]">
                  <li>
                    <button role="menuitemradio" aria-checked={current === "en"} onClick={() => setLang("en")} className={`${current === "en" ? "bg-base-300 text-base-content" : ""} normal-case text-base-content hover:bg-base-300 flex items-center gap-2`}>
                      <span role="img" aria-label="English" className="text-lg">🇬🇧</span>
                      en
                    </button>
                  </li>
                  <li>
                    <button role="menuitemradio" aria-checked={current === "fr"} onClick={() => setLang("fr")} className={`${current === "fr" ? "bg-base-300 text-base-content" : ""} normal-case text-base-content hover:bg-base-300 flex items-center gap-2`}>
